<template>
  <div class="analyt_wrapper" v-if="this.key">
    <h2 class="title">Аналитика</h2>
    <h3 class="subtitle">Аналитика по визитам</h3>
    <div ref="chartdiv" class="diagram"></div>
  </div>
  <div></div>
  <div class="oops_page" v-if="!this.key">
    Ой. Что-то пошло не так...
    <br />
    Чтобы продолжить, перезагрузите страницу.
  </div>
</template>
...
export default {
  ...
  computed: {
    visitData() {
      const formattedData = this.rawVisitData.map((item) => {
        const container = {};
